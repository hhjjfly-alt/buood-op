#!/bin/bash
# diyyb1-part2.sh
# 最终生产精简版（已修复 smartdns 哈希 + 强制禁用 Docker + sing-box 固定稳定版）
...
echo "=== 开始执行 diyyb1-part2.sh（最终生产精简版 - sing-box 固定稳定版） ==="
...
# 5. 注入最新版本 + 强力跳过哈希
##############################################################################
# Sing-Box（固定稳定版，避免最新 1.13.x 的 go-json-experiment 不兼容）
...
#!/bin/bash
# diyyb1-part2.sh
# 最终生产精简版（已修复 smartdns 哈希 + 强制禁用 Docker + 固定 sing-box 稳定版）
# 适配最新 OpenWrt + N6000/PVE + I226
# 保留：中文 + PassWall/PassWall2 + dae + SmartDNS + Lucky + DDNS-GO + Diskman + Tailscale + ksmbd + ttyd + Zerotier
# 强制保留：firewall4 + nftables
# 强制禁用：Docker 全家桶
...
# 5. 注入最新版本 + 强力跳过哈希
##############################################################################
# Sing-Box（固定稳定版，避免 1.13.x 与当前 Go 环境不兼容）
SING_BOX_LATEST="1.12.12"
if [ -f package/passwall-packages/sing-box/Makefile ]; then
    echo "更新 sing-box 到 $SING_BOX_LATEST（固定稳定版，避免编译失败）"
    sed -i "s/^PKG_VERSION:=.*/PKG_VERSION:=$SING_BOX_LATEST/" package/passwall-packages/sing-box/Makefile
